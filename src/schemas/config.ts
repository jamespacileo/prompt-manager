...
export const configSchema = z.object({
		...
		.record(
			z.object({
				temperature: z
					...
					.max(1)
					.optional()
					.describe("AI model temperature parameter"),
				maxTokens: z
					.number()
					.positive()
					.optional()
					.describe("Maximum number of tokens for AI model output"),
				topP: z
					.number()
					.min(0)
					.max(1)
					.optional()
					.describe("Top P parameter for AI model"),
				frequencyPenalty: z
					.number()
					.min(-2)
					.max(2)
					.optional()
					.describe("Frequency penalty for AI model"),
				presencePenalty: z
					.number()
					.min(-2)
					.max(2)
					.optional()
					.describe("Presence penalty for AI model"),
			}),
		)
	...
	verbosity: z
		...
		.max(3)
		.default(1)
		.describe("Verbosity level for logging"),
});
