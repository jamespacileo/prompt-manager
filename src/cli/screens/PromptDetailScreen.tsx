...
const PromptDetailScreen: React.FC<PromptDetailScreenProps> = ({
	...
	const [details, setDetails] = useState<Partial<IPrompt<any, any>> | null>(
		null,
	);
	const [comparisonVersion, setComparisonVersion] = useState<Partial<
		IPrompt<any, any>
	> | null>(null);
	...
	const [generatedTypeScript, setGeneratedTypeScript] = useState<string | null>(
		null,
	);
	...
	useEffect(() => {
		...
		listPromptVersions({ category: prompt.category, name: prompt.name }).then(
			(versions) => {
	...
	useInput((input, key) => {
		if (input === "b" || key.escape) {
			...
			deletePrompt({ category: prompt.category, name: prompt.name }).then(() =>
			...
			const newIndex =
				(currentVersionIndex + (key.leftArrow ? -1 : 1) + versions.length) %
				versions.length;
	...
	return (
		<ScreenWrapper title="Prompt Details">
			...
			<Newline />
			<Text>
				Press {chalk.bold.yellow("b")} to go back, {chalk.bold.yellow("a")} to
				amend, {chalk.bold.yellow("d")} to delete, {chalk.bold.yellow("v")} to
				set current version, {chalk.bold.yellow("t")} to toggle TypeScript view,{" "}
				{chalk.bold.yellow("e")} to evaluate
			</Text>
			{isDeleting && (
				<ConfirmationDialog
			...
			<Box marginTop={1} flexDirection="column">
				...
				<Text>
					Use {chalk.bold.yellow("←")} {chalk.bold.yellow("→")} arrow keys to
					switch versions
				</Text>
			</Box>
		</ScreenWrapper>
