...
describe("promptManagerUtils", () => {
	...
	describe("validateCategoryAndName", () => {
		...
		it("should throw for empty category", () => {
			expect(() => validateCategoryAndName("", "name")).toThrow(
				"Prompt category and name are required and cannot be empty",
			);
		});

		it("should throw for empty name", () => {
			expect(() => validateCategoryAndName("category", "")).toThrow(
				"Prompt category and name are required and cannot be empty",
			);
		});
	});
	...
	describe("handlePromptNotFound", () => {
		it("should throw an error with the correct message", () => {
			expect(() => handlePromptNotFound("category", "name")).toThrow(
				'Prompt "name" in category "category" does not exist',
			);
		});
	});
});
