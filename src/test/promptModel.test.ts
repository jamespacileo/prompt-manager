...
describe("PromptModel", () => {
	...
	test("list prompts", async () => {
		...
		const prompts = await promptManager.listPrompts({
			category: dummyPrompt1.category,
		});

		const filteredPrompt1 = prompts.find(
			(p) =>
				p.name === dummyPrompt1.name && p.category === dummyPrompt1.category,
		);
		expect(filteredPrompt1).toBeTruthy();

		const allPrompts = await promptManager.listPrompts({});
		const filteredPrompt2 = allPrompts.find(
			(p) =>
				p.name === dummyPrompt2.name && p.category === dummyPrompt2.category,
		);
	...
	test("update prompt", async () => {
		...
		const updatedPrompt = promptManager.getPrompt({
			category: dummyPrompt.category,
			name: dummyPrompt.name,
		});
	...
	test("delete prompt", async () => {
		...
		await promptManager.createPrompt({ prompt: dummyPrompt });
		await promptManager.deletePrompt({
			category: dummyPrompt.category,
			name: dummyPrompt.name,
		});
		...
		const deletedPrompt = prompts.find(
			(p) => p.name === dummyPrompt.name && p.category === dummyPrompt.category,
		);
