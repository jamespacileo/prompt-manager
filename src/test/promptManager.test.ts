...
describe("PromptManager", () => {
	...
	test("List all prompts", async () => {
		...
		const createdPrompt = allPrompts.find(
			(p) => p.name === testPrompt.name && p.category === testPrompt.category,
		);
	...
	test("List prompts by category", async () => {
		const testPrompt = getRandomPrompt();
		await manager.createPrompt({ prompt: testPrompt });
		const categoryPrompts = await manager.listPrompts({
			category: testPrompt.category,
		});
		const createdPrompt = categoryPrompts.find(
			(p) => p.name === testPrompt.name,
		);
		expect(createdPrompt).toBeDefined();
	...
	test("Error handling - invalid prompt data", async () => {
		...
		await expect(
			manager.createPrompt({ prompt: invalidPrompt as any }),
		).rejects.toThrow();
	});
});
