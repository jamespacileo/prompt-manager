...
export function validateCategoryAndName(category: string, name: string): void {
	if (!category || !name || category.trim() === "" || name.trim() === "") {
		throw new Error(
			"Prompt category and name are required and cannot be empty",
		);
	}
}

export function mapPromptToFileInfo(
	prompt: PromptModel<any, any>,
	basePath: string,
	...
	return {
		...promptData,
		filePath:
			promptData.category && promptData.name
...
export function handlePromptNotFound(category: string, name: string): never {
	...
	const cleanPromptName = cleanName(name);
	throw new Error(
		`Prompt "${cleanPromptName}" in category "${cleanCategory}" does not exist`,
	);
}

export function getCleanNames(
	category: string,
	name: string,
): { cleanCategory: string; cleanName: string } {
	return {
